...
NDDR NDDR::load(const std::filesystem::path &path)
{
	NDDR nddr;

	std::ifstream fin(path);
	ulog_assert(fin.good(), "Bad nddr file %s\n", path.c_str());

	int32_t sizes[3];
	fin.read(reinterpret_cast <char *> (sizes), sizeof(sizes));
	ulog_info("nddr io", "%d patches, %d vertices, %d feature size\n", sizes[0], sizes[1], sizes[2]);
	...
	nddr.patch_count = sizes[0];
	nddr.feature_size = sizes[2];
	...
	ulog_info("nddr io", "read patches data\n");

	std::array <Tensor, LAYERS> weights;
	for (int32_t i = 0; i < LAYERS; i++) {
		int32_t sizes[2];
		fin.read(reinterpret_cast <char *> (sizes), sizeof(sizes));
		ulog_info("nddr io", "weight matrix with size %d x %d\n", sizes[0], sizes[1]);
	...
	for (int32_t i = 0; i < LAYERS; i++) {
		...
		ulog_info("nddr io", "bias vector with size %d\n", size);
	...
	nddr.patches = patches;
	nddr.features = features;
	nddr.weights = weights;
	nddr.biases = biases;

	// TODO: put this elsewhere..
	// Need special care for vertices to align them properly
	nddr.vertices.resize(vertices.size());
	for (int32_t i = 0; i < vertices.size(); i++)
		nddr.vertices[i] = glm::vec4(vertices[i], 0.0f);
