...
struct HomogenizedNDDR {
	...
	static HomogenizedNDDR from(const NDDR &);
};

HomogenizedNDDR HomogenizedNDDR::from(const NDDR &nddr)
{
	// Concatenate the biases into a single buffer
	std::vector <float> biases;
	for (int32_t i = 0; i < LAYERS; i++)
		biases.insert(biases.end(), nddr.biases[i].begin(), nddr.biases[i].end());
	...
	// Weights (transposed)
	size_t w0c = nddr.weights[0].height;
	std::vector <float> W0(64 * w0c);
	std::vector <float> W1(64 * 64);
	std::vector <float> W2(64 * 64);
	std::vector <float> W3 = nddr.weights[3];

	for (size_t i = 0; i < 64; i++) {
		for (size_t j = 0; j < 64; j++) {
			W1[j * 64 + i] = nddr.weights[1][i * 64 + j];
			W2[j * 64 + i] = nddr.weights[2][i * 64 + j];
		}

		for (size_t j = 0; j < w0c; j++)
			W0[j * 64 + i] = nddr.weights[0][i * w0c + j];
	}

	// Feature vector
	std::vector <glm::vec4> features(nddr.patch_count * nddr.feature_size);

	for (size_t i = 0; i < nddr.patch_count; i++) {
		glm::ivec4 complex = nddr.patches[i];
		for (size_t j = 0; j < nddr.feature_size; j++) {
			float f0 = nddr.features[complex.x * nddr.feature_size + j];
			float f1 = nddr.features[complex.y * nddr.feature_size + j];
			float f2 = nddr.features[complex.z * nddr.feature_size + j];
			float f3 = nddr.features[complex.w * nddr.feature_size + j];
			features[i * nddr.feature_size + j] = glm::vec4(f0, f1, f2, f3);
		}
	}

	return HomogenizedNDDR {
		.patches = nddr.patches,
		.vertices = nddr.vertices,
		.features = features,
		.biases = biases,
		.W0 = W0, .W1 = W1,
		.W2 = W2, .W3 = W3,
...
vk::DescriptorSet neural_geometry_image(DeviceRenderContext &engine, const HomogenizedNDDR &hnddr)
{
	...
	// Allocate textures for the nddr
	uint32_t patches = hnddr.patches.size();
	uint32_t features = hnddr.features.size() / patches;
	uint32_t vertices = hnddr.vertices.size();
	uint32_t biases = hnddr.biases.size();
	uint32_t wsize = hnddr.W0.size() >> 6;

	auto complex_texture = allocator(hnddr.patches, { patches, 1 },
			vk::ImageType::e1D, vk::Format::eR32G32B32A32Sint);

	auto vertex_texture = allocator(hnddr.vertices, { vertices, 1 }, vk::ImageType::e1D);
	auto feature_texture = allocator(hnddr.features, { features, patches });

	auto bias_texture = allocator(hnddr.biases, { biases, 1 }, vk::ImageType::e1D);
	auto W0_texture = allocator(hnddr.W0, { 16, wsize });
	auto W1_texture = allocator(hnddr.W1, { 16, 64 });
	auto W2_texture = allocator(hnddr.W2, { 16, 64 });
	auto W3_texture = allocator(hnddr.W3, { 16, 3 });
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
		ulog_error("testbed", "Usage: testbed <nddr>\n");
		return EXIT_FAILURE;
	}
	...
	auto nddr = NDDR::load(path);
	auto hnddr = HomogenizedNDDR::from(nddr);
	...
	// Initialization
	DeviceRenderContext engine = DeviceRenderContext::from(phdev, extensions, nddr.feature_size);

	engine.camera_transform.position = glm::vec3 { 0, 0, 3 };
	engine.camera_transform.rotation = glm::vec3 { 0, glm::radians(180.0f), 0 };

	// Descriptor sets
	auto meshlet_dset = neural_geometry_image(engine, hnddr);
	...
	stats.patch_count = nddr.patch_count;
	...
	while (valid_window(engine)) {
		...
		cmd.drawMeshTasksEXT(nddr.patch_count, 1, 1);
		...
		if (path.size()) {
			auto nddr = NDDR::load(path);
			auto hnddr = HomogenizedNDDR::from(nddr);
			meshlet_dset = neural_geometry_image(engine, hnddr);
