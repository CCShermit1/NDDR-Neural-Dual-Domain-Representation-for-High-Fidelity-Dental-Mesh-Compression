...
class NDDRTextureFetchFunction(torch.autograd.Function):
    @staticmethod
    def forward(map, u, v, complexes, resx, resy, rate):
        return nddrutil.nddr_texture_fetch_forward(map, u, v, complexes, resx, resy, rate)
    ...
    def backward(ctx, d_color):
        ...
        d_map = nddrutil.nddr_texture_fetch_backward(d_color, u, v, *ctx.rest)
        # print('dMAP', d_map)
        return d_map, None, None, None, None, None, None


class NDDRTextureFetch(nn.Module):
    ...
    def forward(self, map, u, v):
        return NDDRTextureFetchFunction.apply(map, u, v, self.complexes, self.resx, self.resy, self.rate)
