...
def construct_nddr(path: str, config: dict, normalizer: Callable) -> NDDR:
    ...
    return NDDR(points, complexes, features, config)
...
if __name__ == '__main__':
    ...
    for experiment in experiments:
        ...
        nddr = construct_nddr(source_path, local_config, normalizer)
        ...
        # TODO: parameters() method for nddr
        opt = torch.optim.Adam(list(nddr.mlp.parameters()) + [ nddr.points, nddr.features ], lr=1e-3)
        ...
        for rate in [ 4, 8, 16 ]:
            uvs = nddr.sampler(rate)
            V = nddr.eval(*uvs).detach().unsqueeze(0)
            F = nddr(rate)
            ...
            for _ in trange(1000):
                uvs = nddr.sampler(rate)
                nddr_vertices = nddr.eval(*uvs)
                indices = optext.triangulate_shorted(nddr_vertices, nddr.complexes.shape[0], rate)
                faces = remap.remap_device(indices)

                chamfer_loss = chamfer_distance(nddr_vertices.unsqueeze(0), target.vertices.unsqueeze(0))
