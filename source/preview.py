...
def preview_single(nddr, refs):
    mode = list(refs.keys())[0] if (len(refs) > 0) else 'nddr'
    def draw(rate, patches):
        ps.remove_all_structures()

        if nddr is not None and mode == 'nddr':
            sample = nddr.sample_uniform(rate)
            V = nddr.eval(*sample).detach()

            if patches:
                complex_count = nddr.complexes.shape[0]
                ...
                F = nddrutil.triangulate_shorted(V, nddr.complexes.shape[0], rate)
                m = ps.register_surface_mesh('nddr', V.cpu().numpy(), F.cpu().numpy())
    ...
    def callback():
        ...
        if imgui.RadioButton('nddr Model', mode == 'nddr'):
            mode = 'nddr'
            draw(rate, patches)
...
def preview_many(many, refs):
    ...
    def draw(rate):
        ...
        for f, nddr in many.items():
            ...
            indices = nddrutil.triangulate_shorted(V, nddr.complexes.shape[0], rate)
...
def preview_lods(lods):
    ...
    def draw(rate, patches):
        ...
        for name, nddr in lods.items():
            if current != name:
                continue

            V = nddr.eval(rate).detach().cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nddr', type=str, help='path to nddr')
    # parser.add_argument('--many', type=str, nargs='+', help='path to nddrs')
    ...
    nddr = None
    if args.nddr is not None:
        assert os.path.exists(args.nddr)
        nddr = nddr.from_pt(args.nddr)
    ...
    #                 nddr = torch.load(os.path.join(root, file))
    #                 nddr = load_nddr(nddr)
    #                 lods[base] = nddr
    ...
    # else:
    #     assert nddr is not None or refs is not None
    preview_single(nddr, [])
