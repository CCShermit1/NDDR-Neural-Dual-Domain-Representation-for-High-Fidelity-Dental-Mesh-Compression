...
def nddr_size(nddr):
    size = sum([ p.numel() * p.element_size() for p in nddr.parameters()])
    size += nddr.complexes.numel() * nddr.complexes.element_size()
    return size
...
def scene_evaluations(reference, directory, alt=None):
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            print('Loading nddr from', file)

            nddr = torch.load(path)
            nddr = load_nddr(nddr)
            size = nddr_size(nddr)

            uvs = nddr.sample_uniform(16)
            V = nddr.eval(*uvs).detach()

            base = nddr.base(16).detach()
            cmap = make_cmap(nddr.complexes, nddr.points.detach(), base, 16)
            remap = optext.generate_remapper(nddr.complexes.cpu(), cmap, base.shape[0], 16)
            indices = optext.triangulate_shorted(V, nddr.complexes.shape[0], 16)
            F = remap.remap_device(indices)

            nddr_mesh = mesh_from(V, F)

            metrics = evaluator.eval_metrics(nddr_mesh, name)

            metrics['count'] = nddr.complexes.shape[0]
            metrics['size'] = size
...
def nddr_mesh(nddr, rate=16, reduce=True) -> Mesh:
    with torch.no_grad():
        uvs = nddr.sample_uniform(rate)
        V = nddr.eval(*uvs)
        base = nddr.base(rate)

    cmap = make_cmap(nddr.complexes, nddr.points.detach(), base, rate)
    remap = optext.generate_remapper(nddr.complexes.cpu(), cmap, base.shape[0], rate)

    indices = optext.triangulate_shorted(V, nddr.complexes.shape[0], rate)
...
def tessellation_evaluation(prefix):
    ...
    def eval_tessellations(evaluator, nddr, name):
        rate_metrics = {}
        # for rate in [ 2, 4, 8, 12, 16, 32 ]:
        for rate in [ 2, 4, 8, 12, 16 ]:
            mesh = nddr_mesh(nddr, rate)
    ...
    for root, _, files in os.walk(base):
        for file in files:
            if prefix in file and file.endswith('pt'):
                ...
                nddr = torch.load(file)
                nddr = load_nddr(nddr)

                rm = eval_tessellations(evaluator, nddr, scene)
                data[scene] = rm
...
def feature_evaluation(prefix):
    ...
    for root, _, files in os.walk(base):
        for file in files:
            if prefix in file and file.endswith('pt'):
                ...
                features = nddr.features.shape[1]
                setup[scene][features] = nddr
    ...
    for scene, components in setup.items():
        ...
        for k in components:
            ...
            metrics = evaluator.eval_metrics(nddr_mesh, scene)
...
def multichart_evaluations():
    ...
    pattern = re.compile('^lod[1-4].pt$')
    for root, _, files in os.walk(nddr_directory):
        for file in files:
            if pattern.match(file):
                print('Found nddr file', file)
                ...
                mesh = nddr_to_mesh(nddr)
                metrics = evaluator.eval_metrics(mesh)
                del metrics['images']
                print('  > metrics', metrics)

                data['nddr'].append((size, metrics['chamfer']))
...
def frequency_evaluation():
    ...
    for file in glob.glob('results/frequencies/*.pt'):
        ...
        metrics = evl.eval_metrics(nddr_mesh, 'armadillo')
...
def losses_evaluation(directory):
    ...
    nddr_ord = load_nddr(torch.load(ordinary))
    nddr_chm = load_nddr(torch.load(chamfer))

    metrics_ord = evl.eval_metrics(nddr_to_mesh(nddr_ord))
    metrics_chm = evl.eval_metrics(nddr_to_mesh(nddr_chm))
...
def ingp_evaluation():
    ...
    nddr = load_nddr(torch.load(os.path.join(directory, 'primary.pt')))
    nddr_mesh = nddr_to_mesh(nddr)

    print('ref minmax', ref.vertices.min(0)[0], ref.vertices.max(0)[0])
    print('nddr minmax', nddr_mesh.vertices.min(0)[0],nddr_mesh.vertices.max(0)[0])
    print('ingp minmax', ingp_t11.vertices.min(0)[0], ingp_t11.vertices.max(0)[0])

    evl = Evaluator(ref)

    data_nddr = evl.render_everything(nddr_mesh, 'skull')
...
def scroller_evaluation():
    ...
    nddr = load_nddr(torch.load('results/wreck/experimental.pt'))
    nddr_mesh = nddr_to_mesh(nddr)

    evl = Evaluator(ref)
    print('ref, nddr', ref.vertices.shape[0], nddr_mesh.vertices.shape[0])

    return torch.save(evl.render_everything(nddr_mesh, 'wreck'), 'wreck-all.pt')
...
def teaser_evaluation():
    ...
    nddr    = load_nddr(torch.load('evals/teaser/primary.pt'))

    def nddr_to_mesh(nddr, rate=16, reduce=True) -> Mesh:
        with torch.no_grad():
            uvs = nddr.sample_uniform(rate)
            V = nddr.eval(*uvs)
            base = nddr.base(rate)
    ...
    nddr_mesh = nddr_to_mesh(nddr, reduce=False)
    ...
    pindex = torch.arange(nddr.complexes.shape[0]).repeat_interleave(450)
    colors = COLOR_WHEEL[pindex % COLOR_WHEEL.shape[0]]
    patches = evl.renderer.render_false_coloring(nddr.vertices, nddr_mesh.normals, nddr_mesh.faces, colors, camera)[0]
